...
NOTE: How to process videos or images from social media (YT, Twitter)? It is a bit crazy to convert an image into a Tensor by yourself (a Tensor is a three-dimensional matrix where you have the 2D 100x100 pixels image + another dimension of RGB colours (vectors of three n)). So, normally, you would resort to specific functions or packages that are created to address these needs. Afterwards, you could process with CNNs. 
...
# "_": Underscores included
# "+": One or more times. That is, it can pick several hashtags.
...
Download recent tweets from a specific account (the maximum number of most recent tweets that can be freely downloaded per account is ~3200): (Would this change with the Academic API?)
