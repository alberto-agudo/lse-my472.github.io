...
```{r, eval = FALSE}
apikey <- "JEkfG7Wec51dEAjWeN8Mn0qAanp2dcaC"
```
...
# That is, we have the last 10 articles; but the total amount of articles on inequality is 24137.
```

So while the amount of returned documents here is capped at ten, there exists the hits key in the meta data which gives the total amount of articles which contained the keyword. This is very helpful for us and we can use it in a function.
...
nyt_dates_count <- function(q, init, end, by){
  ...
  # Both init and end are assumed in a Date format.
  ...
  # You use the length - 1 to combine it with the last date (dates[i + 1])
  
  # Now the function also returns a dataframe with two columns: date & count
  # dates[-length(dates)] reverses the order of dates
