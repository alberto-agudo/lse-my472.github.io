...
# Inner join between posts and congress.
# Joins them based on the screen name of the politicans.
# You are getting the likes count, the party, and the date.
```
...
dbGetQuery(db, 
  ...
  LIMIT 1")
# The message, name, shares_count and party of the post by a Democrat with the highest number of shares.
...
dbGetQuery(db, 
  ...
  ORDER BY shares_count DESC
  LIMIT 1")
# The message, name, shares_count and party of the post by a Republican with the highest number of shares.
...
# 10 politicians and their posts per name. 
# Group by makes the count function work by group. 
```
...
dbGetQuery(db,
  "SELECT from_name, message, likes_count, comments_count, date,   
      CAST(comments_count AS FLOAT)/likes_count AS comment_like_ratio
  FROM posts
...
```{r}
# Average likes of posts by party
dbGetQuery(db, 
...
# substr(column, index_to_start, number of letters/numbers to extract) 
dbGetQuery(db, 
  "SELECT SUBSTR(date, 9, 2) AS day_of_month, COUNT(*) as post_count
  FROM posts
  GROUP BY day_of_month")
# Here we are looking only at the characters from the ninth onwards, and selecting the two following (so that we select the days).
