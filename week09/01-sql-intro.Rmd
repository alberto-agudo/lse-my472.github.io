...
# Connect function: Searchs a db file, otherwise it creates it.
...
# Check the columns contained in the table
# Get the first five rows from that database.
...
```{r, eval=FALSE}
# Creates a character vector of all file names.
...
for (f in fls){
  ...
  # append = TRUE means that, if there is no such table named posts, it will create it. Afterwards, it will append the new posts to the table.
...
```{r}
# Querying just one column. Selects the name column from the congress table and limits it to the first 10 rows. 
dbGetQuery(db, "SELECT name FROM congress LIMIT 10")

# Multiple columns
dbGetQuery(db, "SELECT name, party FROM congress LIMIT 10")

# Adding expressions (division returns integer). Here you are creating a new column. Upper is another function that gives you the post types in uppercase letters. 
dbGetQuery(db, "SELECT from_name, likes_count/comments_count, UPPER(type) FROM posts LIMIT 10")

# Adding expressions (division returns float). Because dividing an integer between an integer will give you an integer, you can change the type of variable that it is. If you change it into float type, it will return a float division.
dbGetQuery(db, "SELECT from_name, CAST(likes_count AS FLOAT)/comments_count, UPPER(type) FROM posts LIMIT 10")

# Adding aliases to the new columns with AS.
...
# Remember this keyword, it is really important. 
...
# Name and party column from the congress table from Republican politicians.
...
# OR operator
# You do not need to use == to assess equality.
