...
# fill = TRUE allows us to fill the remaining rows that are not filled with data of a column.
...
```
The numeric columns are in character form. 
...
```{r}
tables_raw <- html_nodes(html, css = "#mw-content-text > div.mw-parser-output > table:nth-child(20)")
length(tables_raw)
```
...
p <- ggplot(data_pop, aes(x=rank, y=population, label=city_name)) +
  geom_point() + geom_text(hjust=-.1, size=3)  + #geom_htext adjust text next to the point
  theme_minimal()
p
```
...
p <- ggplot(data_pop, aes(x=rank, y=population, label=city_name)) +
  ...
  scale_y_log10("log(population)", labels=scales::comma) + #the 
  theme_minimal()
p
```

This shows the power laws. 
