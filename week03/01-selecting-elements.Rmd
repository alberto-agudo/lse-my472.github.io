...
html <- read_html(url)
```
read_html() simply parses the html online to an R object. 

html_nodes to get the nodes you want.
...
Why xml_nodeset??
Mainly because xml is a more general implementation of HTML (it does not rely on specific tags or formatting, you can define the tags in the way you want). Hence, most of the packages are built on xml, and they rely on xml to format and parse HTML later. 

If we want to return the text without HMTL tags, we can use the function "html_text()". For example, the first element has the following text (list of nodes):
...
```{r}
relevant_nodes <- html_nodes(html, css = ".division-two> h1:nth-child(1)")
# or ".division-two > p:nth-child(2)"
html_text(relevant_nodes[[1]])
