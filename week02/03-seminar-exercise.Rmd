...
ipu_clean <- ip_and_unemployment %>%
  pivot_wider(id_cols = c("country", "date"), names_from = "series", values_from = "value")  # long to wide

# France
ipu_clean %>%
  filter(country == "france") %>%
  summarise(unemployment = max(unemployment, na.rm = TRUE), 
...
# another way of doing it is finding also the DATES, for this, instead of summarising you are filtering.  
...
ipu_clean %>%
  mutate(ip_sq = ip ** 2,
         unemployment_log = log(unemployment),
