...
Normally you use conditionals for webscraping to automatize processes.
...
for (i in 1:5) {
  ...
  # stringsAsFactors = FALSE is used so that the characters that appear in the data frame are not considered factors
  grades[[i]] <- student
}
# Now we have a list of single line data frames...
class(grades)
# We can combine these into a joint data frame
#do.call rbinds all the elements from the list
...
A key conceptual feature of functions is that they have their own environments (local scope). What do you think the following cell will return?
...
The 20 output only exists within the function, because the function is local and not global. 
...
# Using an ifelse function to return an absolute value. The same as np.where in Python
