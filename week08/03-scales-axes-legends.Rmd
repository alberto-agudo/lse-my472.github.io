...
# Here you can see how the breaks stop after your last specified break.
...
# Here you could also specify which label corresponds to which discrete value. 
...
# Take into account that these colours and shapes do not appear inside the aes() argument.
...
# Here you are changing the way in which the legend is displayed.
# If you do not want the legend to be shown, then: + theme(legend.position = "none")
```
...
You can also use ggthemes for further themes. 
...
dev.off()
# You'll always need to use dev.off() to close the code.
```
