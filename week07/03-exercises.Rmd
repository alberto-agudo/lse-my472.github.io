...
```{r}
uol_postcodes <- read_file("uol.txt") %>% str_replace_all("\\n", " ")
str_view_all(uol_postcodes, "[A-Z]{1,2}\\d\\w?\\s?\\d[A-Z]{2}")
```
...
```{r}
str_replace_all(uol_postcodes, "([A-Z]{1,2}\\d\\w?)\\s?\\d[A-Z]{2}", "\\1")
# Here you are replacing all the regex by the first captured group (which lasts until the \\w?).
# Within Atom you could also do it with $1. (You replace all the sequence by the firts captured group).
# If you want to specifiy a second capture group, you need to specify it before. 
...
Imagine that you have a graph where one coordinate shows the number of times on word has been mentioned in a text, whereas the other coordinate would show the frequency of another word. To comput ethe similarity of two documents between these two words we draw a line from the 0-0 coordinate to each point, and the angle between both lines shows us the differences (not the lenght of the lines because that is affected by the size of the document). If the angle is really large then there would be big differences. The similarity measure would mean doing it for n-dimensions ()
