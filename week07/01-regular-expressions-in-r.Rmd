...
In regex you can also use look aheads and look afters of several patterns (for instance, extract everything that comes after the...)
...
```
stringr already appears inside tidyverse.

Specify the directory where the txt file is stored:

```{r}
current_dir <- "/Users/agudo07/Documents/LSE MSc Applied Social Data Science/courses/1 - MT/MY472 - DATA FOR DATA SCIENTISTS/lse-my472.github.io/week07/"
```

Load the txt file into a character vector with `read_file`:

```{r}
sample_text <- read_file(paste(current_dir, "sample.txt", sep = ""))
```
...
View selects the first occurrence. 
...
It returns a list because it can be applied to a list of strings.
