...
Computational side: Python has more implementations of Deep Learning applied to NLP.

Even though there are several ways to write the same regex query, some of them are more efficient. How do you know which is the most efficient one? You should time the execution of your code. 

Some people have already built dictionaries within their field of expertise (like financial dictionaries, psychologists have also created dictionaries for emotions...)
...
Docvars in this case is specified as metainformation.
...
dfmplain
```
Sparse means that the table contains a lot of zeros (91.8%) because some words do not appear in many documents. 
...
```{r}
# General wordcloud for all presidents
textplot_wordcloud(dfmcleaned, 
                   rotation=0, min_size=.75, max_size=3, max_words=50)

# Here you are filtering the dfm only with speeches of Obama.
...
dfmcleaned <- data_corpus_inaugural %>%
  ...
  # the padding argument: When you remove stopwords, it replaces them with empty strings so that you can get bigrams that make sense (otherwise, you would get a bigram that did not exist before, since you would have eliminated the stopword that separated them).
  # bigrams, combinations of two words. Bigrams are sequences of two words. 
  tokens_ngrams(n = 1:2) %>% # up to bigrams
  dfm() %>%
  dfm_trim(min_termfreq = 2)
# each feature should appear in the corpus at least twice. 
dfmcleaned
```
