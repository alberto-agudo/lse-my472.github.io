...
Overhead, distributing the tasks between a number of cores also takes some time.
...
# Serial execution
# The .combine argument would have the same behaviour as a do.call(rbind).
system.time({
  foreach (i = 1:3, .combine=rbind) %do% {   # %do% = serial execution
    sqrt(i)
  }
})
...
# Parallel execution
system.time({
  foreach (i = 1:3, .combine=rbind) %dopar% {   # %dopar% = parallel execution
    sqrt(i)
  }
})
