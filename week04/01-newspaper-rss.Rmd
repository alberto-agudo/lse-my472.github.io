...
Next, let us prototype how you could scrape the text in the body of each of those URLs. We pick the first URL and write some code to get an object that contains the text of the article. You could select the main object that holds all the information, but its CSS selector changes from one webpage to another; so we select all paragraphs in the file.
...
test_text <- test_url %>%
  ...
  html_nodes(css = "p") %>% 
  html_text() %>%
  # You combine all those paragraphs with a newline to differentiate between paragraphs.
  paste(collapse = "\n")
...
for (i in 1:nrow(data_guardian_articles_small)) {
  ...
  # Obtain URL of current article from the tibble
