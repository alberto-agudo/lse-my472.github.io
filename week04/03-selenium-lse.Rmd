...
```{r}
cookies <- driver$findElement(using = "xpath", value = "//div[1]/p/a[1]")
...
# Substitutes what comes after the hyphen by nothing.

last_result_this_page_test <- as.numeric(gsub(".*-","", split_string_test[2]))
# Substitutes what comes before the hyphen by nothing.
...
current_result_counts <- function() {
  ...
  # The equivalent to string split in Python. 
...
scrape_programmes <- function(term) {
  ...
  while (last_page_flag == FALSE) {
    ...
    for (programme_int in 1:programmes_on_this_page) {
      ...
      current_programe_xpath <- sprintf("//div[1]/article[%g]/a/header/h1", programme_int)
