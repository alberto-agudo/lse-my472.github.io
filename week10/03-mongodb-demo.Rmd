...
for (f in fls){
  ...
  # For each group of posts, you join the information of the congress data with the info of the posts dataset (you are creating duplicates).
...
# The id field appears because it is used to identify each document (row).
# I think that the query('{}') is like the where, it filters rows. 
...
```{r}
# Here it is saying: . (any character), {2} two times.
# Also the $options: "i" is referring to case-insensitive. 
...
# Here you see that "$sum":1, it means that we count each row with 1. 
...
Here sort specifies the order_by clause of SQL. 1 Means ascending order and -1 descending order. 
...
# The project operator works as field when there is no group variable. 
# Here aggregate is used because it is necessary when there is any sort of operation. 
collection$aggregate('[{"$match" : {"likes_count": {"$gt": 1000}}},
                  {"$project": {"from_name": true, "message": true, "likes_count": true, "comments_count": true, "date": true, "party":true,
...
Here:
$project shows you which fields you select. 
$lookup (command to start the join operator.
$localField: Field on your local collection that you will use as a joining key.
$from: is the right-hand collection that you'll be joining. 
$foreignField: foreign key
$as: additional_data (name of the collection included in your original collection).

Because it is not a tabular dataset, each document contains another document. Each field can save entire documents or JSON objects.
Additional_data is a dataframe. 
...
mongo = collection$find(query = '{"type": {"$ne": "photo"}, "likes_count": {"$gt": 500}}', 
        fields = '{"from_name": true, "type": true, "date": true, "likes_count": true}', 
        limit = 10), times = 200)
# Average time in Mongo is twice as large.
# It specifies the unit of output.
```
...
It is expressed in milliseconds. Already averaged out.
