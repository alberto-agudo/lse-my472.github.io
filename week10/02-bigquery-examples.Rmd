...
These are not toy datasets, maybe they are not complete but still allow you some analysis of historical data. 
...
# The DBI package wraps the bigquery query inside.
# It is a more convenient wrapping than in Python. 
db
```
...
# normalized time series of crime per borough
...
shp <- readOGR(dsn = "data/statistical-gis-boundaries-london/ESRI/LSOA_2011_London_gen_MHW.shp",
               stringsAsFactors = FALSE)
# the readOGR function belongs to the rgdal package
...
data_plot_3 <- shp@data %>%
  ...
  # Here it is using a Pythonic index because it is the shape that the shp file has.
  
  # merge automatically uses an inner join. 
...
```{r}
# TIMESTAMP_DIFF, difference between end and beginning, stores the difference in minutes. 
dbGetQuery(db, "SELECT AVG( TIMESTAMP_DIFF(stoptime, starttime, MINUTE) )
           AS duration_minutes
           FROM citibike_trips")

# Or also:
dbGetQuery(db, "SELECT AVG(tripduration) / 60
           AS duration_minutes
           FROM citibike_trips")
```
...
```{r}
# Why do you multiply latitude + longitude by 111?
...
Females tend to cycle longer distances, which explains the differences in the minutes of bike rides. 